...
<head>
    ...
    <title>Prestasi - SD N 3 Srikandang</title>
...
<body class="bg-white">
    ...
    @php
    $heroImage = asset('assets/img/lomba senam harapan 3 tingkat kecamatan.jpeg');
    $heroImagePosition = 'center 65%';
    $heroImageScale = 1.05;
@endphp
@include('partials.page-hero', [
    'title' => 'Prestasi Membanggakan',
    'subtitle' => 'Pencapaian siswa-siswi kami di berbagai kompetisi',
    'image' => $heroImage,
        ...
        <div class="container mx-auto px-6">
            @php
                ...
                $placeholders = [
                    ...
                    ['Juara 1 Lomba Tahfidz', 'Tingkat Kecamatan 2024'],
                    ['Juara 2 Lomba Melukis', 'Tingkat Provinsi 2024'],
                    ['Juara 3 Turnamen Futsal', 'Tingkat Kabupaten 2023'],
                    ['Juara 1 Lomba Literasi', 'Tingkat Kecamatan 2024'],
                    ['Juara 2 Festival Seni', 'Tingkat Kabupaten 2023'],
                ];
            @endphp
            ...
            <div class="grid md:grid-cols-3 gap-6 mb-6">
                @forelse ($achievements as $achievement)
                    <div class="card-hover rounded-2xl overflow-hidden shadow-xl h-64 bg-gray-100 relative">
                        <img src="{{ $achievement->image_path ? asset('storage/' . $achievement->image_path) : $fallbackImage }}" alt="{{ $achievement->title }}" class="w-full h-full object-cover">
                        <div class="absolute inset-0 bg-gradient-to-t from-black/60 via-black/10 to-transparent"></div>
                        <div class="absolute bottom-0 left-0 right-0 p-4 text-white">
                            <p class="font-semibold flex items-center gap-2">
                                <img src="{{ asset('assets/icons/prestasi.svg') }}" alt="" aria-hidden="true" class="w-5 h-5">
                                <span>{{ $achievement->title }}</span>
                            </p>
                            @if ($achievement->subtitle)
                                <p class="text-xs text-white/80">{{ $achievement->subtitle }}</p>
                            @endif
                        </div>
                    </div>
                @empty
                    @foreach ($placeholders as [$title, $subtitle])
                        <div class="card-hover rounded-2xl overflow-hidden shadow-xl h-64 bg-gray-100 relative">
            ...
            <div class="bg-gradient-to-br from-green-700 to-emerald-700 rounded-3xl p-10 text-white">
                <h3 class="text-2xl font-bold mb-8 text-center">Ringkasan Prestasi</h3>
                <div class="grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
                    <div><div class="text-4xl font-bold mb-2">{{ $stats['total'] }}</div><div class="text-emerald-100">Total Prestasi</div></div>
                    <div><div class="text-4xl font-bold mb-2">{{ $stats['juara_1'] }}</div><div class="text-emerald-100">Juara 1</div></div>
                    <div><div class="text-4xl font-bold mb-2">{{ $stats['provinsi'] }}</div><div class="text-emerald-100">Tingkat Provinsi</div></div>
                    <div><div class="text-4xl font-bold mb-2">{{ $stats['tahun_aktif'] }}</div><div class="text-emerald-100">Tahun Aktif</div></div>
                </div>
            </div>
        </div>
    </section>
